...
namespace BlockReplace
{
	...
	/// <summary>
	/// Description of MystWorld.
	/// </summary>
	public class MystWorld : Substrate.NbtWorld
	{		
        ...
        public RegionChunkManager GetMystChunkManager(int dim)
        {
        	return GetMystChunkManagerVirt(dim) as RegionChunkManager;
        }
        ...
        public static new MystWorld Open (string path)
        {
            return new MystWorld().OpenWorld(path) as MystWorld;
        }
        ...
        /// <returns>A new <see cref="BetaWorld"/> object representing an existing world on disk.</returns>
        public static MystWorld Open (string path, int cacheSize)
        {
            MystWorld world = new MystWorld().OpenWorld(path);
        ...
        public static MystWorld Create (string path)
        {
            return new MystWorld().CreateWorld(path) as MystWorld;
        }
        ...
        /// <remarks>This method will attempt to create the specified directory immediately if it does not exist, but will not
        /// write out any world data unless it is explicitly saved at a later time.</remarks>
        public static MystWorld Create (string path, int cacheSize)
        {
            MystWorld world = new MystWorld().CreateWorld(path);
            world._prefCacheSize = cacheSize;
        ...
        protected IChunkManager GetMystChunkManagerVirt (int dim)
        {
            RegionChunkManager rm;
            if (_chunkMgrs.TryGetValue(dim, out rm)) {
                return rm;
        ...
        /// <exclude/>
        protected override Substrate.Data.DataManager GetDataManagerVirt ()
        {
            if (_dataMan != null) {
                return _dataMan;
            }
        ...
        private void OpenMystDimension (int dim)
        {
            string path = Path;
            if (dim == Dimension.DEFAULT) {
                path = IO.Path.Combine(path, _REGION_DIR);
                ...
                path = IO.Path.Combine(path, "DIM_MYST" + dim);
                path = IO.Path.Combine(path, _REGION_DIR);
            }
        ...
        private MystWorld OpenWorld (string path)
        {
            if (!Directory.Exists(path)) {
                if (File.Exists(path)) {
        ...
        private MystWorld CreateWorld (string path)
        {
            if (!Directory.Exists(path)) {
                throw new DirectoryNotFoundException("Directory '" + path + "' not found");
            }
        ...
        {
            try {
                MystWorld world = new MystWorld().OpenWorld(e.Path);
                if (world == null) {
                    return;
                }
